...
from custom_components.roommind_eklabs.coordinator import RoomMindCoordinator
from custom_components.roommind_eklabs.managers.heat_source_orchestrator import (
    DeviceCommand,
    HeatSourcePlan,
)
...
def coordinator(hass, mock_config_entry, real_store):
    hass.data = {"roommind_eklabs": {"store": real_store}}
...
class TestHeatSourceIntegration:
    ...
    async def test_ekf_training_uses_adjusted_power_fraction(self, coordinator, real_store, hass):
        ...
        with patch(
            "custom_components.roommind_eklabs.coordinator.evaluate_heat_sources",
